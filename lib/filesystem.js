...
export async function parseBlogPost(indexPath, baseUrl, blogPathPrefix) {
  const raw = await fs.promises.readFile(indexPath, 'utf8')
  const lines = raw.split(/\r?\n/)
  const { data: frontmatter, bodyLines } = parseFrontmatter(lines)
  const body = normalizeMarkdownAssets(bodyLines.join('\n'), baseUrl, blogPathPrefix)
  const bodyLineList = body.split(/\r?\n/)
  const freeLines = []
  const paywallLines = []
  let paywallStarted = false
  let paywallTier = null
  let title = null
  let summary = ''

  for (const line of bodyLineList) {
    if (/^## Paywall: [A-Za-z ]+$/.test(line)) {
      paywallTier = line.slice(12).trim().toLowerCase()
      paywallStarted = true
      continue
    }
    if (paywallStarted) {
      paywallLines.push(line)
    } else {
      freeLines.push(line)
    }
    if (!paywallStarted) {
  ...
  const content = renderMarkdown(freeLines.join('\n'))
  const paywallContent = paywallLines.length ? renderMarkdown(paywallLines.join('\n')) : null

  return { frontmatter, title, summary, content, paywallContent, paywallTier }
}
