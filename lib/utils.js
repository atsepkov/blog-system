...
export function normalizeMarkdownAssets(markdown, baseUrl, blogPathPrefix) {
  ...
  // Rewrite bare slug links to use blog path prefix
  // Matches [text](word-word) but not URLs, absolute paths, anchors, or file extensions
  if (blogPathPrefix) {
    const prefix = blogPathPrefix.replace(/\/+$/, '')
    updated = updated.replace(/\[([^\]]+)\]\(([a-z0-9][a-z0-9-]*[a-z0-9])\)/g, (_match, text, slug) => {
